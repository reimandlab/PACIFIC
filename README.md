...
## Example run 1:
As a simple example, let's search for prognostic mutation features in this dataset, without including any other control variable in the model. Run `pipeline_step1()` for 20 iterations:
```R
set.seed(1) # for reproducibility of this demo
pipeline_step1(data = dataset$data,
               single_features = dataset$mutation_features,
               num_iterations = 20, 
               output_dir='out')
```
Now run `pipeline_step2()`:
```R
results <- pipeline_step2(step1_output_dir = 'out',
                          plot_km = TRUE)
```
...
```
Now run `pipeline_step2()` with assigning the clinical features to the baseline variables for ANOVA tests:
```R
results <- pipeline_step2(step1_output_dir = 'out',
